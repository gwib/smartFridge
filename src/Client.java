 ...
 * ---------
 * 
 * - all Clients running in the same PC ("localhost") with different ports and generated with the same main
 */
...
public class Client {
	...
	private static String directions = "\nType the number of the action and then hit Enter.\n\n"
			...
			+ " 7     "+REMOVERECIPIE+"\n"
			+ " 8     "+MODIFYRECIPIE+"\n"
			+ " 9     "+GETPOSSIBLERECIPESTODO+"\n"
			+ " 10    "+GETRECIPESWITHINGREDIENTSABOUTTOEXPIRE+"\n"
			+ "\nOther commands:\n"
			+ " d     print directions\n"
	...
	private String[] rpcComm(String function, String[] args) {
		...
		try {
			...
			//log.info(sdf.format(new Date()) + " - " + "Couldn't reach the Client: "+e.getMessage()+".");
		}
		return reply;
	}
	
	private void readConsole(Client sl) throws InterruptedException {
		setConsola(null);
		sl.params = null;
	...
	private void changeIp(Client sl){
		System.out.println("Will be able to change destination IP,\nlike it can be done with the port.");
	}
	
	private void changePort(Client sl) throws InterruptedException {
		setConsola(null);
	...
	private boolean isPositiveInteger(Client sl, String parameter) throws InterruptedException {
	...
	private boolean isPositiveDouble(Client sl, String parameter) throws InterruptedException {
		String errorMsg = "\n  Must be a double > 0.\n  " + parameter + ": ";
		setConsola(null);
		while(getConsola() == null) {
			readConsole(sl);
	...
	private void validateDate(Client sl, String parameter) throws InterruptedException {
	...
	private void addItems(Client sl) throws InterruptedException {
	...
	private void getListItems(Client sl) throws InterruptedException {
		//System.out.println("Sorry, we are still working on it! ;)");
	...
	private void chooseItems(Client sl) throws InterruptedException {
		System.out.println("Sorry, we are still working on it! ;)");
	...
	private void getShoppingList(Client sl) {
		String[] reply = sl.rpcComm(sl.GETSHOPPINGLIST, new String[] {""});
		System.out.println(reply[0]);
	}
	
	private void addRecipe(Client sl) {
		System.out.println("Sorry, we are still working on it! ;)");
	}
	
	private String strArray2str(String [] recipeList) {
		String list = "";
		int i = 1;
		for(String r: recipeList) {
			//System.out.println(recipeList);
			
			String[] recipe = r.split(";");
			list += "\n" + Integer.toString(i) + ". Name: " + recipe[0] + "\n   Ingredients:";
			i++;
			String[] ingredients = recipe[1].split(":");
			int j = 1;
			for (String ing: ingredients) {
				String[] ingValues = ing.split(",");
				System.out.println(ing);
				//list += "\n   " + Integer.toString(j) + ". " + ingValues[0] + ", " + ingValues[1];
				j++;
			}
	...
	private void getRecipes(Client sl) {
		//System.out.println("Sorry, we are still working on it! ;)");
		String[] reply = sl.rpcComm(sl.GETRECIPES, new String[] {});
		System.out.println("List of recipes:\n"+strArray2str(reply));
	}
	
	private void removeRecip(Client sl) {
		System.out.println("Sorry, we are still working on it! ;)");
	}
	private void modifyRecip(Client sl) {
		System.out.println("Sorry, we are still working on it! ;)");
	}
	private void getPossibleRecipesToDo(Client sl) {
		String[] reply = sl.rpcComm(sl.GETPOSSIBLERECIPESTODO, new String[] {});
		System.out.println("Possible recipes to do:\n"+strArray2str(reply));
	}
	private void getRecipesWithIngredientsAboutToExpire(Client sl) {
		String[] reply = sl.rpcComm(sl.GETRECIPESWITHINGREDIENTSABOUTTOEXPIRE, new String[] {});
		System.out.println("Possible recipes to do with items about to expire:\n"+strArray2str(reply));
	}
	
	private void testComm(Client sl) {
    	...
    	while(isRunning) {
        	...
        	if(sl.consola != null) {
        		...
        		switch (sl.params[0]) {
			        ...
			        case "9":  sl.getPossibleRecipesToDo(sl); break;
			        case "10":  sl.getRecipesWithIngredientsAboutToExpire(sl); break;
